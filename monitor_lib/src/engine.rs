...
pub async fn simulate_server_metrics(server: &Server) -> (i32, f32, f32) {
    if server.status == "OFF" || server.status == "/" || server.status == "STOPPING" {
        return (0, 0.0, 0.0);
    }
...
-
    let (efficiency_multiplier, base_load) = match server.cpu_model.to_lowercase().as_str() {
    ...
    let cpu = (base_load + (cpu_wave * 15.0 * efficiency_multiplier) + cpu_jitter).clamp(1.0, 99.0) as f32;

    let ram_offset = (server.id as f64) * 9876.54;
    let ram_wave = ((now + ram_offset) / 12000.0).sin();
    
    let server_specific_base_ram = 0.2 + ((server.id % 5) as f32 * 0.1); 
    let ram_percent = (server_specific_base_ram + (ram_wave as f32 * 0.15)).clamp(0.1, 0.9);
    
    let ram = (server.max_ram * ram_percent).clamp(0.1, server.max_ram);

    let final_ms = if server.port == 0 { -1 } else { ms.max(1) };
